...
### 新功能
- **FDA 大结果集预警**（P1-11）：FDA 宽查询（无日期范围或条件过宽）时 openFDA 可能返回数千上万条申请记录，全量预取耗时长且占内存。现检测到结果集超阈值（约 2000 条 / 20 页）时弹一次确认框，提示添加或缩小日期范围，可即时取消（复用现有取消链路 `_cancel_search`），避免无谓的全量拉取

### 修复
- **CDE 进度条预估虚高**（P1-10）：CDE 下载进度条此前用「详情页数×2」预估文件总数（假设每页有审评报告+说明书 2 个 PDF），但实际详情页可能只有 1 个或无 PDF，导致进度条 maximum 虚高、解析阶段全程停在 0。现 Phase 1 按「待解析详情页数」推进（含解析失败的 error 路径）、Phase 2 按实际提取到的 PDF 数重设进度条，确认对话框文案改为「预计至多 N×2，实际以解析结果为准」
...
### 新功能
- **FDA 跨页勾选**：勾选状态以 `doc_url` 为键外部存储，翻页时自动恢复，不再丢失已勾选的行
- **FDA 跨页全选**：「全选」按钮支持跨全部页面选中所有符合当前筛选条件的行（如筛选 doc_type=Medical Review 后全选，仅选中 Medical Review 行）
- **FDA 筛选计数**：分页标签显示筛选相关计数，如「第 1-200 条 / 筛选 2188 条，共 4709 条」；`filters_changed` 信号实时更新

### 修复
- **FDA/CDE 下载跳过空转延迟**：下载器逐个检查文件是否存在，跳过时仍经历 8-15 秒（FDA）/ 5-10 秒（CDE）的下载间隔延迟。改为下载前预扫描目录，已有文件直接标记跳过不进入延迟队列，与提取页行为一致
- **FDA 下载文件名中文**：下载文件名将 doc_type 映射为中文（如 "Review" → "综合审评"），改为保持英文原名
- **FDA 翻页重置筛选条件**：`FilterTableView.set_data()` 每次清空表头筛选，翻页调用 `set_data()` 导致筛选丢失。新增 `keep_filters` 参数，翻页时保留筛选

### 改进
- **下载进度显示**：FDA 下载启动时显示预扫描结果（如「预扫描: 5 个已存在，正在下载 187 个文件」），进度条仅计实际下载量
- **代码复用**：FDA 筛选逻辑提取为 `_get_filtered_urls()` 公共方法，`_select_all()` 和 `_update_page_label()` 共用，消除重复代码

### CDE 页面改进
- **CDE 默认全量爬取**：移除「爬取全部页」复选框，搜索时默认自动翻页爬取所有记录，无需手动勾选
- **CDE 日期提前终止**：设了起始日期时，当某页全部记录都早于该日期则立即停止爬取（如设 2025-01-01 可能只需爬 10 页而非 184 页）
- **CDE 分页计数修正**：修复「第 41-90 条，共 50 条」的计数矛盾，改为直接显示「共 X 条记录」
- **CDE 注册分类说明**：分页标签右侧新增「注册分类说明」按钮，弹窗展示 NMPA 化学药品 1-5 类注册分类含义
